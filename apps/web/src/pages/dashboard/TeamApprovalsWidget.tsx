import { Banknote, Calendar, Check, ClipboardCheck, Clock, FileText, Plane, X } from "lucide-react";
...
function Row({
  ...
  return (
    <div className="flex items-center gap-3 rounded-lg px-3 py-2.5 bg-bg transition-colors duration-150 hover:bg-border/40">
      ...
      <button
        disabled={busy}
        onClick={onApprove}
        className="w-7 h-7 rounded-lg flex items-center justify-center text-success bg-successSoft transition-all duration-150 hover:brightness-95 active:scale-95 disabled:opacity-50 disabled:pointer-events-none cursor-pointer disabled:cursor-not-allowed"
        aria-label="تأیید"
      >
        <Check size={14} />
      </button>
      <button
        disabled={busy}
        onClick={onReject}
        className="w-7 h-7 rounded-lg flex items-center justify-center text-danger bg-danger/10 transition-all duration-150 hover:brightness-95 active:scale-95 disabled:opacity-50 disabled:pointer-events-none cursor-pointer disabled:cursor-not-allowed"
        aria-label="رد"
      >
        <X size={14} />
      </button>
    ...
    <div className="rounded-xl p-5 bg-surface border border-border shadow-card">
      <div className="flex items-center gap-2.5 mb-3.5">
        ...
        <p className="text-sm font-bold text-textPrimary tracking-tight">
          تأییدیه‌های تیم من
          <span className="text-textSecondary font-medium"> ({total})</span>
        </p>
      </div>
